...
def hard_clips(scores):
    ...
    # Erosion
    erosion_large = ndimage.binary_erosion(scores_bin, structure=np.ones((1, 10))).astype(scores_bin.dtype)
...
def IVCL(logits, text_features, visual_features, labels, lengths, device):
    ...
    for i in range(logits.shape[0]):
        tmp_top, idx_top = torch.topk(scores_region_inner[i], k=int(logits.shape[1] / 16 + 1), largest=True)
...
def COSL(soft_features, hard_features):
    ...
    scores = F.cosine_similarity(soft_features, hard_features, dim=-1, eps=1e-07)
...
def train(model, normal_loader, anomaly_loader, test_loader, args, label_map, device):
    ...
    for e in range(args.max_epoch):
        ...
        for i in range(min(len(normal_loader), len(anomaly_loader))):
            ...
            loss = loss1 + loss2 + loss3 * 1.0 + loss4 * 1e-1 + loss5 * 1e-1
