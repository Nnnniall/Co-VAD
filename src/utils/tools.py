...
def get_prompt_text(label_map: dict, json_path: str):
    with open(json_path, "r", encoding="utf-8") as f:
        feature_json = json.load(f)

    prompt_class = list(label_map.values())

    prompt_text = {cls: feature_json.get(cls, "") for cls in prompt_class}

    return prompt_class, prompt_text
...
def save_best_record(epoch, step, auc, ap, map, file_path):
    fo = open(file_path, "w")
    fo.write("epoch: {}\n".format(epoch))
    fo.write("step: {}\n".format(step))
    fo.write("auc: {:.4f}\n".format(auc))
