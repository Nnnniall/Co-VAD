...
def refine_scores_hierarchical(logits_mlp, logits_align, temp=1.0, epsilon=1e-12):
...
def test(model, testdataloader, maxlen, prompt_class, prompt_text, gt, gtsegments, gtlabels, device):
    ...
    with torch.no_grad():
        for i, item in enumerate(testdataloader):
            ...
            optimized_probs = refine_scores_hierarchical(logits1[0:len_cur], logits2[0:len_cur], 1.0)
    ...
    best_auc, best_ap = (ROC1, AP1) if AP1 >= AP2 else (ROC2, AP2)
...
if __name__ == '__main__':
    ...
    test(model, test_loader, args.visual_length, prompt_class, prompt_text, gt, gtsegments, gtlabels, device)
