...
def refine_scores_hierarchical(logits_mlp, logits_align, temp=5.0, epsilon=1e-12):
...
def test(model, testdataloader, maxlen, prompt_class, prompt_text, gt, gtsegments, gtlabels, device):
    ...
    with torch.no_grad():
        for i, item in enumerate(testdataloader):
            ...
            _, _, _, logits1, logits2 = model(visual, padding_mask, prompt_class, prompt_text, lengths)
            ...
            optimized_probs = refine_scores_hierarchical(logits1[0:len_cur], logits2[0:len_cur], 5.0)
    ...
    best_auc, best_ap = (ROC1, AP1) if ROC1 >= ROC2 else (ROC2, AP2)
    ...
    return best_auc, best_ap, averageMAP
...
if __name__ == '__main__':
    ...
    label_map = dict({'Normal': 'normal', 'Abuse': 'abuse', 'Arrest': 'arrest', 'Arson': 'arson', 'Assault': 'assault',
                      ...
                      'RoadAccidents': 'road accidents', 'Robbery': 'robbery', 'Shooting': 'shooting',
    ...
    testdataloader = DataLoader(testdataset, batch_size=1, shuffle=False, num_workers=4, pin_memory=True)
    ...
    model = CLIPVAD(args.classes_num, args.embed_dim, args.visual_length, args.visual_width, args.visual_head, args.visual_layers, args.attn_window, args.prompt_prefix, args.prompt_postfix, args.gate_blend, device)
    model_param = torch.load(args.model_path)
    model.load_state_dict(model_param)

    test(model, testdataloader, args.visual_length, prompt_class, prompt_text, gt, gtsegments, gtlabels, device)
